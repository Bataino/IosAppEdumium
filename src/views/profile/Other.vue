  ...
  <ion-page>
    <ion-content class="text-start">
      <div class="text-start">
        <div class="d-flex justify-content-between text-start text-sm px-3 p-2" v-for="(detail, key, index) in details"
          :key="detail" :ke="index" style="max-width:400px">
          <div class="w-100 ion-text-capitalize">
            <!-- {{ key.replace('_', ' ') }} : -->
            {{ key.split('_').join(' ') }} :
          </div>
          <div class="text-start fw-bold w-100" style="width:100px">
            {{ detail }}
          </div>
        </div>
      </div>
